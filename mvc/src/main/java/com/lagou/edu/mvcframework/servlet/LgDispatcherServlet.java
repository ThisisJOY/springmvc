            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if (method.isAnnotationPresent(LagouRequestMapping.class)) {
                    ...
                    if (method.isAnnotationPresent(Security.class)) {
                        ...
                        String[] methodUsernames = security.value(); // {"zhangsan1, lisi1"}

                        String[] usernames = intersect(controllerUserNames, methodUsernames);// Controller和方法都允许的username交集
                        handler.setUsername(usernames);
                    } else {
                        handler.setUsername(controllerUserNames);
                    }
...
//        method.invoke() //
        ...
        // 验证权限
        final String[] allowedUsernames = handler.getUsername();
        List<String> list = Arrays.asList(allowedUsernames);

        //获取请求中的用户名
        final Integer usernameIndex = handler.getParamIndexMapping().get("username"); //username=1&username=2   username [1,2]
        final String reqUsername = (String) paraValues[usernameIndex]; // [1,2]

        List<String> reqUsernameList = new ArrayList<>();
        if (reqUsername.contains(",")) {
            reqUsernameList = Arrays.asList(reqUsername.split(",")); //请求中有多个username 比如username=1&username=2
        } else {
            reqUsernameList.add(reqUsername); // 只有一个username
        }


        for (int i = 0; i < reqUsernameList.size(); i++) {
            String s = reqUsernameList.get(i);
            if (!list.contains(s)) {
                return; // 用户名验证不通过，返回
            }
        }
